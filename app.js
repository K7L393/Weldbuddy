...
function sendFollowUp() {
    ...
    const API_KEY = "AIzaSyC1DHTraXG7xqzayZ4eAmfGzW9tKEvZp2U"; 
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${API_KEY}`;

    const systemInstruction = `You are WeldCoach, an elite industrial structural steel welding mentor. 
    ...
    .catch(error => {
        ...
        thread.innerHTML += `
            <div class="bg-rose-950/20 p-4 rounded-xl border border-rose-900/30 mt-2 text-rose-300 text-sm font-mono">
                🚨 Error reaching AI Engine. Confirm your API Key is pasted correctly inside app.js.
            </div>
        `;
        scrollContainer.scrollTop = scrollContainer.scrollHeight;
    });
